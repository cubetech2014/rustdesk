...
//
// UI 에 대하여:
//   첫 버전은 MessageBoxW 만 쓴다. TaskDialog 의 명령 링크가 더 보기 좋지만,
//   TASKDIALOGCONFIG 는 union 이 섞인 큰 구조체라 로컬 컴파일 없이 한 번에
//   맞히기 어렵다. 배관(아키텍처 판별 - 조회 - 다운로드 - 실행)이 실제로 도는
//   것을 먼저 확인하고, UI 는 그 다음에 다듬는다.
...
/// 사용자가 무엇을 원하는지 묻는다.
///
/// 버튼 라벨이 예/아니오/취소 로 고정이라 본문에서 각각이 무엇인지 명시한다.
/// 보기 좋진 않지만 모호하지 않고, 어떤 Windows 에서도 똑같이 동작한다.
#[cfg(windows)]
